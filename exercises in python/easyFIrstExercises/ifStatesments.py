...
# CR: In these two methods, you have basically the same name - it is impossible from the method names to distinguish between the functionality of check_if_positive and isPositive. You need to give methods names that are indicative to what they do, even if it is something along the lines of test_check_if_positive.
...
def is_triangle(num1, num2, num3):
    ...
    # CR: Are you sure this code is correct? I remember that the sum of every two sides should be larger than the remaining third, not one set (a+b>c and b+c>a and c+a>b)
...
def six_is_great(num1, num2):
    # CR: An if statement like this is extremely confusing, and therefor extremely error prone. You can split it into several lines using \ (defining a multiline statement in Python), or actually create variables: 
    ...
    # CR: The number 6 repeats itself. Consider the trouble of changing the number 6 to 9 (and also the probability of missing one) - you should save it in a variable: magic = 6.
    # For extra credit, you could also put it as an optional parameter in your method, enabling the same method to generically evaluate different members: magic_is_great(num1, num2, magic=6)
